...
// Special function to get auth directly - for use in login page
export async function getFirebaseAuth() {
  if (firebaseAuth) {
    return firebaseAuth
  }

  // If auth isn't initialized yet, try to initialize Firebase
  try {
  ...
  // If that fails, try to get auth directly
  try {
    const { getAuth } = await import("firebase/auth")
    return getAuth()
  } catch (error) {
    console.error("Error getting Firebase Auth directly:", error)
    return null
  }
}
